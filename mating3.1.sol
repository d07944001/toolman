...
contract Mating {
   ...
   function withdrawToolCoin(uint amount) public payable {
    ...
    function callSpecificService (address _toolman, uint number) public{
        require(callingService[_toolman][msg.sender] == false);
        callingService[_toolman][msg.sender] = true;
        emit callService (msg.sender, _toolman, number);
    }
    function rejectService (address lazyman) public{
